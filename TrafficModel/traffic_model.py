...
def initialize_tree():
    ...
    try:
        cur.execute("""SELECT location_road, location_bound, location_area, timestamp, traffic FROM entries WHERE extract(hour from update_timestamp) > 12 AND extract(hour from update_timestamp) < 14""")
    except:
